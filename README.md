...
if current users count is bigger than defaultUsers for the new plan then we calculate the extra price

> extra price = ( current users - defaultUsers ) \* pricePerUser

and the new formula would be like this:

> price = upgraded plan price + extra price - (remaining days \* current plan price / 30)

note: we calculate a discount based on the remaining days instead of just substracting the current plan price
